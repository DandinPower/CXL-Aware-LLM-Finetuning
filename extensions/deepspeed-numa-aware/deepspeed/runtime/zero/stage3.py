...
from deepspeed.utils import zeros_cpu_for_master_weights, zeros_cpu_for_master_gradients, zeros_cpu_for_compute_weights, zeros_cpu_for_compute_gradients
...
class DeepSpeedZeroOptimizer_Stage3(ZeROOptimizer):
    ...
    def _setup_for_real_optimizer(self):
        ...
        self.grad_partitions_flat_buffer = zeros_cpu_for_compute_gradients(shape=(sum(p.partition_numel() for p in all_params),), dtype=self.gradient_accumulation_dtype, pin_memory=self.offload_optimizer_pin_memory)
    ...
    def _create_param_groups_fp16_flat_cpu_memory(self):
        ...
        for j, param_group in enumerate(self.trainable_param_groups):
            ...
            if flat_buffer_size > 0:
                ...
                self.param_groups_fp16_flat_cpu_memory.append(zeros_cpu_for_compute_weights(shape=(int(flat_buffer_size),), dtype=self.dtype, pin_memory=True))
            else:
            ...
            if self._swappable_optimizer_subgroup(i):
                ...
                if self.params_in_nvme_and_cpu and tensor is None:
                    ...
                    if self.offload_optimizer:
                        ...
                        fp32_partitioned_group_flat_tensor = zeros_cpu_for_master_weights(self.fp16_partitioned_groups_flat[i].shape, torch.float, False)
                        fp32_partitioned_group_flat_tensor.copy_(self.fp16_partitioned_groups_flat[i])
                        self.fp32_partitioned_groups_flat.append(fp32_partitioned_group_flat_tensor)                   
                    else:
                        self.fp32_partitioned_groups_flat.append(self.fp16_partitioned_groups_flat[i].to(
    ...
    def initialize_optimizer_states(self):
        ...
        for i, group in enumerate(self.fp16_groups):
            ...
            if self.offload_optimizer and not swappable_optimizer_subgroup:
                ...
                self.fp32_partitioned_groups_flat[i].grad = zeros_cpu_for_master_gradients(self.fp32_partitioned_groups_flat[i].shape, gradient_dtype, self.offload_optimizer_pin_memory)
            else:
