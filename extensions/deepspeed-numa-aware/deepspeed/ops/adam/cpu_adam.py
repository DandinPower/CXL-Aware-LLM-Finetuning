...
from deepspeed.utils import zeros_cpu_for_variances, zeros_cpu_for_momentums
        ...
        for group_id, group in enumerate(self.param_groups):
            for param_id, p in enumerate(group['params']):
                ...
                if len(state) == 0:
                    ...
                    # gradient momentums
                    state['exp_avg'] = zeros_cpu_for_momentums(shape=p.data.shape, dtype=state_dtype, pin_memory=False)
                    # gradient variances
                    state['exp_avg_sq'] = zeros_cpu_for_variances(shape=p.data.shape, dtype=state_dtype, pin_memory=False)
