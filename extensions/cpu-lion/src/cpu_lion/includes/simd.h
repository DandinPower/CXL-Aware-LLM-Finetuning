...
template <int span>
inline void simd_sign(AVX_Data* dst, AVX_Data* src)
{
#if defined(__AVX512__)
...
#pragma unroll
    for (size_t i = 0; i < span; ++i) {
        __mmask16 pos_mask = _mm512_cmp_ps_mask(src[i].data, zero, _CMP_GT_OQ);
        __mmask16 neg_mask = _mm512_cmp_ps_mask(src[i].data, zero, _CMP_LT_OQ);
        __m512 out = _mm512_setzero_ps();
        out = _mm512_mask_mov_ps(out, pos_mask, one);
        out = _mm512_mask_mov_ps(out, neg_mask, neg_one);
        dst[i].data = out;
    }
#elif defined(__AVX256__)
...
#pragma unroll
    for (size_t i = 0; i < span; ++i) {
        __m256 pos_mask = _mm256_cmp_ps(src[i].data, zero, _CMP_GT_OQ);
        __m256 neg_mask = _mm256_cmp_ps(src[i].data, zero, _CMP_LT_OQ);
        __m256 pos_val = _mm256_and_ps(pos_mask, one);
        __m256 neg_val = _mm256_and_ps(neg_mask, neg_one);
        dst[i].data = _mm256_or_ps(pos_val, neg_val);
    }
#endif
}
